...
namespace JetBrains.Refasmer
{
    public partial class MetadataImporter
    {
        ...
        {
            var ctorHandle = FindMethod(NotImplementedExceptionName, ".ctor", CheckNotImplementedCtorSignature, out var arReader);

            if (!IsNil(ctorHandle))
            {
                Trace?.Invoke($"Found NotImplementedException constructor {_reader.ToString(ctorHandle)}");
                return arReader == _reader ? Import(ctorHandle) : ImportFromReferencedAssembly(ctorHandle, arReader);
            }
